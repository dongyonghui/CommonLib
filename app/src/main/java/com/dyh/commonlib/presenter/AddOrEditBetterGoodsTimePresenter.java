 ...
 * 新建或编辑优选菜展示时间
 */
public class AddOrEditBetterGoodsTimePresenter extends MVPPresenter<IDefaultOptionView> {


    public AddOrEditBetterGoodsTimePresenter( IDefaultOptionView view) {
        super(view);
    }
    ...
    public void addOrEdit(TimeItemBean requestBean) {
        String url = ApiPathConstants.ADD_BETTER_GOODS_TIME;
        if (!TextUtils.isEmpty(requestBean.getId())) {
            url = ApiPathConstants.EDIT_BETTER_GOODS_TIME;
        }
        EasyHttp.post(url)
